...
module.exports = function (templateDirectories, outputFile, opts, done) {
	...
	async.waterfall([
		...
		function (callback) {
			...
			fs.appendFileSync(outputFile, "    " + minify_sync(pug.runtime.merge.toString()).code + '\r\n')
			fs.appendFileSync(outputFile, "    " + minify_sync(pug.runtime.classes.toString()).code + '\r\n')
			fs.appendFileSync(outputFile, "    " + minify_sync(pug.runtime.style.toString()).code + '\r\n')
			fs.appendFileSync(outputFile, "    " + minify_sync(pug.runtime.attr.toString()).code + '\r\n')
			fs.appendFileSync(outputFile, "    " + minify_sync(pug.runtime.attrs.toString()).code + '\r\n')
			fs.appendFileSync(outputFile, "    " + minify_sync(pug.runtime.escape.toString()).code.replace('pug_match_html', '(/[\"&<>]/)') + '\r\n')
			fs.appendFileSync(outputFile, "    " + minify_sync(pug.runtime.rethrow.toString()).code + '\r\n')
			fs.appendFileSync(outputFile, "    var pug = {\r\n")
			fs.appendFileSync(outputFile, "    	merge:" + minify_sync(pug.runtime.merge.toString()).code + ',\r\n')
			fs.appendFileSync(outputFile, "    	classes:" + minify_sync(pug.runtime.classes.toString()).code + ',\r\n')
			fs.appendFileSync(outputFile, "    	style:" + minify_sync(pug.runtime.style.toString()).code + ',\r\n')
			fs.appendFileSync(outputFile, "    	attr:" + minify_sync(pug.runtime.attr.toString()).code + ',\r\n')
			fs.appendFileSync(outputFile, "    	attrs:" + minify_sync(pug.runtime.attrs.toString()).code + ',\r\n')
			fs.appendFileSync(outputFile, "    	escape:" + minify_sync(pug.runtime.escape.toString()).code.replace('pug_match_html', '(/[\"&<>]/)') + ',\r\n')
			fs.appendFileSync(outputFile, "    	rethrow:" + minify_sync(pug.runtime.rethrow.toString()).code + '\r\n')
			fs.appendFileSync(outputFile, "    }\r\n")
			fs.appendFileSync(outputFile, "\r\n")
							...
							if (stats && stats.isDirectory()) {
								...
								if (ext == '.pug') {
									...
									buildTemplateFromFile(filepath, options, function (err, template) {
										pugatizerPath += minify_sync(template.toString()).code + ';\r\n\r\n'
										fs.appendFileSync(outputFile, pugatizerPath)
										finishFile(i)
									})
								} else {
